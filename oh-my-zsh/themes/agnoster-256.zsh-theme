...
prompt_segment() {
  ...
  [[ -n $1 ]] && bg="$1m"
  [[ -n $2 ]] && fg="$2m"
  if [[ $CURRENT_BG != 'NONE' && $bg != $CURRENT_BG ]]; then
    echo -n " $FG_PREFIX$CURRENT_BG$BG_PREFIX$bg$SEGMENT_SEPARATOR$FG_PREFIX$fg "
  elif [[ $CURRENT_BG != 'NONE' && $bg = $CURRENT_BG ]]; then
    echo -n " $BG_PREFIX$bg$SUBSEGMENT_SEPARATOR$FG_PREFIX$fg "
  else
    echo -n "$BG_PREFIX$bg$FG_PREFIX$fg "
  fi
  CURRENT_BG=$bg
  [[ -n $3 ]] && echo -n $3
}
...
prompt_context() {
  if [[ "$USER" != "$DEFAULT_USER" || -n "$SSH_CLIENT" ]]; then
    prompt_segment black default "%(!.%{%F{yellow}%}.)[%{$fg[cyan]%}%n%{$fg[default]%}@%{$fg[magenta]%}%m%{$fg[default]%}]"
  fi
}
...
prompt_virtualenv() {
  ...
  if [[ -n $virtualenv_path && -n $VIRTUAL_ENV_DISABLE_PROMPT ]]; then
    prompt_segment $white $default "(`basename $virtualenv_path`)"
  fi
}
